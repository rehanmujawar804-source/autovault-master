...
export default function PrintableInvoice({ invoice, shopSettings }: PrintableInvoiceProps) {
  ...
  // Generate blank spacer rows to maintain constant height (optimized for single A4 page)
  const MIN_ROWS = 5;
  ...
  return (
    <div
      id="invoice-print"
      className="bg-white text-slate-900 border-4 border-double border-navy-950 p-5 rounded-xl w-full font-sans select-text shadow-sm max-w-[210mm] mx-auto print:border-4 print:p-5 print:m-0 print:rounded-none print:shadow-none print:max-w-full"
    >
      {/* Void Warning Header */}
      {invoice.voided && (
        <div className="border-4 border-red-500 rounded-xl p-3 mb-4 bg-red-50 text-red-700 flex flex-col items-center justify-center text-center print:border-red-500 print:bg-red-50 print:text-red-700">
          <span className="text-base font-black tracking-widest uppercase">VOIDED INVOICE</span>
      ...
      <div className="border-b border-slate-200 pb-3 mb-3">
        {/* Memo Info Row */}
        <div className="flex justify-between items-center text-[10px] text-slate-500 font-semibold uppercase tracking-wider mb-1.5">
        ...
        <div className="flex flex-col items-center justify-center text-center mt-2">
          <div className="flex items-center justify-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-black tracking-tight text-navy-950 uppercase font-sans">
              {shopName}
            </h1>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 mb-4 bg-slate-50/70 border border-slate-100 p-3 rounded-xl print:bg-white print:p-0 print:border-none print:mb-3">
      ...
      <div className="border border-slate-300 rounded-lg overflow-hidden mb-3 print:border-slate-400">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-100/80 border-b border-slate-300 text-xs font-bold text-slate-700 uppercase print:bg-slate-50">
              <th className="py-2 px-3 text-center border-r border-slate-300 w-[8%] print:border-slate-400">Sr.</th>
              <th className="py-2 px-4 border-r border-slate-300 w-[57%] print:border-slate-400">Particulars / Product Name</th>
              <th className="py-2 px-3 text-center border-r border-slate-300 w-[10%] print:border-slate-400">Qty</th>
              <th className="py-2 px-4 text-right border-r border-slate-300 w-[12%] print:border-slate-400">Rate (₹)</th>
              <th className="py-2 px-4 text-right w-[13%]">Amount (₹)</th>
            </tr>
          </thead>
          <tbody className="text-sm text-slate-800 font-medium">
            {invoice.items.map((item, idx) => (
              <tr key={idx} className="border-b border-slate-200 last:border-b-0 hover:bg-slate-50/50 print:border-slate-300">
                <td className="py-2 px-3 text-center border-r border-slate-200 font-mono text-xs text-slate-400 print:border-slate-300">{idx + 1}</td>
                <td className="py-2 px-4 border-r border-slate-200 font-semibold print:border-slate-300">{item.name}</td>
                <td className="py-2 px-3 text-center border-r border-slate-200 font-mono print:border-slate-300">{item.quantity}</td>
                <td className="py-2 px-4 text-right border-r border-slate-200 font-mono print:border-slate-300">₹{item.price.toLocaleString()}</td>
            ...
            {emptyRows.map((_, idx) => (
              <tr key={`empty-${idx}`} className="border-b border-slate-200 last:border-b-0 min-h-[28px] print:border-slate-300">
                <td className="py-1.5 px-3 border-r border-slate-200 text-center text-xs text-slate-300 print:border-slate-300">{invoice.items.length + idx + 1}</td>
                <td className="py-1.5 px-4 border-r border-slate-200 print:border-slate-300">&nbsp;</td>
                <td className="py-1.5 px-3 border-r border-slate-200 print:border-slate-300">&nbsp;</td>
                <td className="py-1.5 px-4 border-r border-slate-200 print:border-slate-300">&nbsp;</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 border-b border-slate-200 pb-3 mb-3">
        {/* SECTION E: PAYMENT / BILL META BLOCK (Left side) */}
        <div className="md:col-span-7 space-y-2">
          <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-3 space-y-1.5 text-xs text-slate-600 print:bg-white print:border-none print:p-0">
            <h4 className="font-extrabold text-[10px] uppercase tracking-wider text-slate-400 mb-1 border-b border-slate-100 pb-0.5">Payment &amp; Audit Info</h4>
        ...
        <div className="md:col-span-5 space-y-1 text-sm font-medium">
          ...
          <div className="flex justify-between text-xs font-bold border-b border-slate-100 pb-1 text-slate-700">
      ...
      <div className="flex items-baseline mb-4 text-xs">
      ...
      <div className="pt-3">
        <div className="grid grid-cols-3 items-end text-center">
          ...
          <div className="space-y-3 text-right flex flex-col items-end">
            ...
            <div className="border-b border-slate-300 w-32 md:w-40 mt-4 print:border-slate-400" />
        ...
        <div className="text-center text-[9px] text-slate-400 font-medium tracking-wide mt-4 print:mt-3">
          {footerMessage}
        </div>
      </div>
    </div>
